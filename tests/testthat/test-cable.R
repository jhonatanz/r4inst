test_that("cable initialization works", {
  ...
  cable_obj <- cable$new("cable-001")
  expect_equal(cable_obj$tag, "cable-001")

  # Test that initialization with an empty tag throws an error
  expect_error(cable$new(""), "Tag for cable should have at least one character")
})
...
test_that("generate_structure works", {
  ...
  cable_obj$generate_structure("pair", 3, TRUE, TRUE)
  ...
  expect_equal(length(cable_obj$struc), 4) # 3 pairs + 1 overall shield
  expect_equal(names(cable_obj$struc), c("PR1", "PR2", "PR3", "OV_SHD"))

  # Test a cable with conductors, no individual shields, and no overall shield
  cable_obj$generate_structure("cond", 5, FALSE, FALSE)
  ...
  expect_equal(length(cable_obj$struc), 5)
  expect_equal(names(cable_obj$struc), c("CO1", "CO2", "CO3", "CO4", "CO5"))
